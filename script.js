const flavors = {
  chocolate: { index: "01", name: "Chocolate", image: "flavor-chocolate.webp", back: "CHOCO", front: "LATE", bg: "#e97cac", accent: "#f5b820", description: "Intenso, cremoso y listo para cambiarte el dia." },
  ddl: { index: "02", name: "Dulce de leche", image: "flavor-ddl.webp", back: "DULCE", front: "DE LECHE", bg: "#d49a66", accent: "#f7cf4d", description: "El clasico argentino, llevado a frecuencia fit." },
  frutilla: { index: "03", name: "Frutilla", image: "flavor-frutilla.webp", back: "FRUTI", front: "LLA", bg: "#ed789b", accent: "#f7d856", description: "Frutal, cremosa y siempre de buen humor." },
  frutos: { index: "04", name: "Frutos del bosque", image: "flavor-frutos.webp", back: "BERRY", front: "MOOD", bg: "#9d7ad2", accent: "#e84273", description: "Frutal, vibrante y con el punto justo de acidez." },
  mani: { index: "05", name: "Crema de mani", image: "flavor-mani.webp", back: "MANI", front: "POWER", bg: "#efaa46", accent: "#ec5668", description: "Cremosa, potente y peligrosamente cucharable." },
  limon: { index: "06", name: "Mousse de limon", image: "flavor-limon.webp", back: "MOUSSE", front: "LIMON", bg: "#f3dc68", accent: "#f4a629", description: "Aireada, citrica y refrescante hasta la ultima cucharada." },
  vainilla: { index: "07", name: "Vainilla", image: "flavor-vainilla.webp", back: "VAINI", front: "LLA", bg: "#efd9ac", accent: "#6f3f2d", description: "Suave, perfumada y con toda la cremosidad de un clasico." },
  coco: { index: "08", name: "Coco", image: "flavor-coco.webp", back: "COCO", front: "FRESH", bg: "#87c9df", accent: "#f7f0df", description: "Liviano, tropical y listo para bajar un cambio." },
  banana: { index: "09", name: "Banana", image: "flavor-banana.webp", back: "BANA", front: "NA", bg: "#f5c645", accent: "#e84884", description: "Dulce, suave y con energia de tarde soleada." }
};
...
const stores = window.FROSZ_STORES || [];

const storeGrid = document.querySelector("#store-grid");
const storesCount = document.querySelector("#stores-count");
const storesMore = document.querySelector("#stores-more");
const storeSearch = document.querySelector("#store-search");
const storesPerPage = 12;
let activeStoreZone = "all";
let visibleStoreLimit = storesPerPage;
let storeQuery = "";
...
stores.sort((a, b) => {
  const byLocality = a[2].localeCompare(b[2], "es", { sensitivity: "base" });
...
stores.forEach(([name, address, locality, zone = "caba"]) => {
  ...
  card.dataset.search = normalizeStoreText(`${name} ${address} ${locality}`);
  localityLabel.textContent = locality;
  title.textContent = name;
  location.textContent = address;

  card.append(localityLabel, title, location);
...
const updateStoreView = () => {
  ...
  const matchingCards = cards.filter((card) => {
    const matchesZone = activeStoreZone === "all" || card.dataset.zone === activeStoreZone;
    const matchesQuery = !storeQuery || card.dataset.search.includes(storeQuery);
    return matchesZone && matchesQuery;
  });

  cards.forEach((card) => { card.hidden = true; });
  matchingCards.slice(0, visibleStoreLimit).forEach((card) => { card.hidden = false; });

  const visibleStores = Math.min(visibleStoreLimit, matchingCards.length);
  const remainingStores = Math.max(0, matchingCards.length - visibleStores);
  storesCount.textContent = matchingCards.length
    ? `Mostrando ${visibleStores} de ${matchingCards.length} ${matchingCards.length === 1 ? "punto" : "puntos"}`
    : "No encontramos puntos con esa busqueda";
  storesMore.hidden = remainingStores === 0;
  storesMore.innerHTML = remainingStores > storesPerPage
    ? `VER ${storesPerPage} MAS <span aria-hidden="true">↓</span>`
    : `VER ${remainingStores} MAS <span aria-hidden="true">↓</span>`;
};
...
document.querySelectorAll(".store-filter").forEach((button) => {
  button.addEventListener("click", () => {
    ...
    activeStoreZone = button.dataset.zone;
    visibleStoreLimit = storesPerPage;
    updateStoreView();
  });
});

storesMore.addEventListener("click", () => {
  visibleStoreLimit += storesPerPage;
  updateStoreView();
});

storeSearch.addEventListener("input", () => {
  storeQuery = normalizeStoreText(storeSearch.value);
  visibleStoreLimit = storesPerPage;
  updateStoreView();
});
